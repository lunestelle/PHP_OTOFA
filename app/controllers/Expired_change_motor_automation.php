        ...
        foreach ($tricyclesExpiredChangeMotor as $expired) {
          ...
          if ($notificationInterval >= 15) {
            ...
            if ($currentStatusIndex !== false && end($existingStatuses) !== 'Dropped' && $currentStatusIndex < count($sequence) - 1) {
              // first the next status in the sequence
              ...
              if ($nextStatus === 'Dropped') {
                ...
                if ($nextStatus === "2nd Notice") {
                  $noticeNo = "Expired Motor (2nd Notice)";
                  $this->sendNotification($user, $tricycleApplicationData, $noticeNo);
                } elseif ($nextStatus === "Expired Motor (3rd Notice)") {
                  $noticeNo = "3rd Notice";
                  $this->sendNotification($user, $tricycleApplicationData, $noticeNo);
                }
