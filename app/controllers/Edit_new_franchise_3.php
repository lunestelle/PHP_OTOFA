...
class Edit_new_franchise_3
{
  ...
  public function index()
  {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if (isset($_POST['confirm_delete_image'])) {
        ...
        if (file_exists($originalImagePath)) {
          ...
          if ($deleted) {
            ...
            redirect('edit_new_franchise_3?appointment_id=' . $appointmentId);
          } else {
      ...
      if (isset($_POST['update_new_franchise'])) {
        $formErrors = $this->validateAppointmentAndTricycleFormData($appointmentFormData, $tricycleApplicationFormData1, $tricycleApplicationFormData2, $tricycleApplicationFormData3, $appointmentModel, $tricycleApplicationModel,  $availableCinNumbers);

        if (!empty($formErrors)) {
          $firstError = reset($formErrors);
          set_flash_message($firstError[0], "error");
          $data = array_merge($data, $_POST);
          echo $this->renderView('edit_new_franchise_3', true, $data);
          return;
        } else {
          ...
          $fileUploads3 = $this->handleFileUploads($mtopRequirementFormData, '3');

          if ($appointmentModel->update(['appointment_id' => $appointmentId], $appointmentFormData) && $tricycleApplicationModel->update(['tricycle_application_id' => $tricycleApplicationDataArray[0]->tricycle_application_id], $tricycleApplicationFormData1) && $tricycleApplicationModel->update(['tricycle_application_id' => $tricycleApplicationDataArray[1]->tricycle_application_id], $tricycleApplicationFormData2) && $tricycleApplicationModel->update(['tricycle_application_id' => $tricycleApplicationDataArray[2]->tricycle_application_id], $tricycleApplicationFormData3)) {
            if (!empty($fileUploads1) && !empty($fileUploads2) && !empty($fileUploads3)) {
              ...
              $mtopRequirementModel->update(['mtop_requirement_id' => $mtopRequirementDataArray[2]->mtop_requirement_id], $fileUploads3);
            }
            ...
            $selectedCinNumberId3 = $tricycleApplicationFormData3['tricycle_cin_number_id'];
            ...
            if (!empty($selectedCinNumberId3) && in_array($selectedCinNumberId3, $availableCinNumbers)) {
              // Update the tricycle_cin_numbers table
              $tricycleCinNumberModel->update(['tricycle_cin_number_id' => $selectedCinNumberId3], [
                'is_used' => true,
                'user_id' => $appointmentData->user_id,
                'ownership_date' => date('Y-m-d'),
              ]);
            ...
            if (!empty($selectedCinNumberId3) && !empty($tricycleApplicationDataArray[2]->tricycle_cin_number_id) &&
            $selectedCinNumberId3 != $tricycleApplicationDataArray[2]->tricycle_cin_number_id) {
              // Update the previous tricycle_cin_numbers entry
              $tricycleCinNumberModel->update(['tricycle_cin_number_id' => $tricycleApplicationDataArray[2]->tricycle_cin_number_id], [
                'is_used' => false,
                'user_id' => null,
                'ownership_date' => null, 
              ]);
            ...
            if ($appointmentFormData['status'] == 'Completed') {
              ...
              $tricycleData3 = [
                'cin_id' =>  $tricycleApplicationDataArray[2]->tricycle_cin_number_id,
                'tricycle_application_id' => $tricycleApplicationDataArray[2]->tricycle_application_id,
                'mtop_requirements_new_franchise_id' => $mtopRequirementDataArray[2]->mtop_requirement_id,
                'user_id' => $appointmentData->user_id,
              ];
              ...
              if ($tricycleModel->insert($tricycleData1)) {
                ...
                if ($tricycleModel->insert($tricycleData2)) {
                  ...
                  if ($tricycleModel->insert($tricycleData3)) {
                    $tricycleId3 = $tricycleModel->getLastInsertedRecord()[0]->tricycle_id;
                    $tricycleStatusesModel->insert(['tricycle_id' => $tricycleId3, 'user_id' => $appointmentData->user_id, 'status' => 'Active']);
                  }
                }
              }
            }
            ...
            list($cinNumber1, $cinNumber2, $cinNumber3) = array_pad($cinNumbers, 3, null);
            ...
            if ($cinNumber3 === null) {
              $cinNumber3 = $tricycleApplicationFormData3['tricycle_cin_number_id'];
            }
            ...
            $customTextMessage = $this->generateCustomTextMessage($appointmentFormData['name'], $appointmentFormData['appointment_type'], $formattedDate, $formattedTime, $rootPath, [$cinNumber1, $cinNumber2, $cinNumber3]);
            
            $customEmailMessage = $this->generateCustomEmailMessage($formattedDate, $formattedTime, $appointmentFormData['appointment_type'], [$cinNumber1, $cinNumber2, $cinNumber3]);
            
            $customRequirementMessage = $this->generateCustomRequirementMessage();
            
            sendAppointmentNotifications($appointmentFormData, $data, $tricycleApplicationFormData1, [$cinNumber1, $cinNumber2, $cinNumber3], $customTextMessage, $customEmailMessage, $customRequirementMessage);
  ...
  private function validateAppointmentAndTricycleFormData($appointmentFormData, $tricycleApplicationFormData1, $tricycleApplicationFormData2, $tricycleApplicationFormData3, $appointmentModel, $tricycleApplicationModel,  $availableCinNumbers) {
    $errors = array();
    ...
    if ($_SESSION['USER']->role === 'admin'){
      if ($appointmentFormData['status'] === 'Completed' || $appointmentFormData['status'] === 'Approved') {
        ...
        } else if (empty($cinNumber3)) {
          $errors['tricycleApplication3'][] = 'Tricycle CIN number is required and must <br> be selected from the available options to <br> update this appointment request.';
        }
      }
    }
